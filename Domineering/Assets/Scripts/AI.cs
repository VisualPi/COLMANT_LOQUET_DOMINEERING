...
public class AI {
    ...
    public bool Move(Board b) {
        ...
        switch (_algo) {
            ...
            case EAiAlgo.ALPHABETA:
                ...
                if (NegamaxAlphaBeta(3, alpha, beta, b, out outCoord, EPlayer.HORIZONTAL) >= 0)
                    retCoord = outCoord;
                break;
            case EAiAlgo.ITERATEALPHABETA:
                if (IterateAlphaBeta(b, out outCoord, EPlayer.HORIZONTAL, 5000f) >= 0)
                    retCoord = outCoord;
                break;
            case EAiAlgo.KILLER:
                baseDepth = 3;
    ...
    public List<Coordonnee> SimulateMove(Board b, bool simulatePlayer)//if true, simulate player's turn
    {
        ...
        if (simulatePlayer) {
            if (this._direction == EPlayer.HORIZONTAL) {//joueur est vertical
    ...
    public int Negamax(int depth, Board b, out Coordonnee c, EPlayer player) {
        ...
        int eval = -2;
        Coordonnee dumb;
        foreach (var move in SimulateMove(b, player != EPlayer.HORIZONTAL)) {
            Play(b, move, player != EPlayer.HORIZONTAL);
            int e = -Negamax(depth - 1, b, out dumb, player == EPlayer.HORIZONTAL ? EPlayer.VERTICAL : EPlayer.HORIZONTAL);
            Undo(b, move, player != EPlayer.HORIZONTAL);
            if (e > eval) {
                c = move;
                eval = e;
            }
    ...
    public int IterateAlphaBeta(Board b, out Coordonnee c, EPlayer player, float timer) {
        var bTmp = new Board(b.GetLines(), b.GetColumns());
        bTmp.SetBoard(b.GetBoard());
        ...
        do {
            var retTmp = -1;
            var c2 = new Coordonnee();
            var thread = new Thread(() => { retTmp = NegamaxAlphaBeta(i, -10000, 10000, bTmp, out c2, player); });

            thread.Start();

            if (thread.Join((int)(timer - t.ElapsedMilliseconds))) {
                if (retTmp >= 0) {
                    retValue = retTmp;
                    c = c2;
                }
    ...
    public int NegamaxAlphaBetaKiller(int depth, int alpha, int beta, Board b, out Coordonnee c, EPlayer player) {
        ...
        int e;
        Coordonnee dumb;
        List<Coordonnee> sims = SimulateMove(b, player != EPlayer.HORIZONTAL);
